...
*Status: goedgekeurd ontwerp; **M1 t.e.m. M4 zijn klaar** — v0.1.0 is publiceerbaar via HACS (custom repository). Datum: 18 augustus 2026. Herzien: meter-first i.p.v. calc-first.*
...
| **M4** ✅ | HACS-klaar: hacs.json, README met automation-voorbeelden (laadpaal pauzeren op `kwartier_marge`), GitHub Actions | Klaar: hacs.json, hassfest lokaal groen, workflows validate/tests, MIT-licentie, GitHub-repo `johan71gent/ha-capacity-tariff` |
